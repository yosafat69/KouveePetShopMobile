...
public class Transaksi_Adapter extends RecyclerView.Adapter<Transaksi_Adapter.ViewProcessHolder> implements Filterable {
    private ArrayList<TransaksiDAO> item;
    private ArrayList<TransaksiDAO> itemFilterd;
    private Context mContext;
    private SharedPrefManager sharedPrefManager;

    public Transaksi_Adapter(Context context, ArrayList<TransaksiDAO> item) {
        this.item = item;
        ...
        holder.itemList.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (data.isTransaksiLayanan == 0) {
                    Intent intent = new Intent(mContext, TransaksiPenjualanKeranjang.class);
                    intent.putExtra("id", data.id);
                    intent.putExtra("isTransaksiLayanan", data.isTransaksiLayanan);
                    sharedPrefManager = new SharedPrefManager(mContext);
                    sharedPrefManager.saveSPInt("spIdTransaksi", data.id);
                    ((Activity) mContext).startActivityForResult(intent, 1);
                }
                else {
                    Intent intent = new Intent(mContext, TransaksiLayananKeranjang.class);
                    intent.putExtra("id", data.id);
                    intent.putExtra("isTransaksiLayanan", data.isTransaksiLayanan);
                    sharedPrefManager = new SharedPrefManager(mContext);
                    sharedPrefManager.saveSPInt("spIdTransaksi", data.id);
